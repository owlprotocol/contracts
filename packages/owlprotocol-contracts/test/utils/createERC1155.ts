...
// Creates + returns dummy ERC1155 tokens for use in testing
export async function createERC1155(tokens = 1, signer?: SignerWithAddress) {
    ...
    for (let i = 0; i < tokens; i++) {
        if (signer) contracts.push(FactoryERC1155.connect(signer).deploy(`Coin ${i}`, mintAmount) as Promise<FactoryERC1155>);
        else contracts.push(FactoryERC1155.deploy(`Coin ${i}`, mintAmount) as Promise<FactoryERC1155>);
    }
